...
export const markRead = mutation({ args: { notificationId: v.id("notifications") }, handler: async (ctx, { notificationId }) => { const { user } = await currentUser(ctx); const item = await ctx.db.get(notificationId); if (!item || item.userId !== user._id) throw new Error("FORBIDDEN"); await ctx.db.patch(item._id, { read: true }); return true; } });
