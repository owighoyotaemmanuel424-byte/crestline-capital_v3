...
export const requestWithdrawal = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const id = await ctx.db.insert("withdrawalRequests", {
      userId: user._id,
      accountId: account._id,
      amount: args.amount,
      currency: account.currency,
...
export const fundingQueue = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.type === "deposit") {
      return await ctx.db.query("depositRequests").withIndex("by_status", (q) => q.eq("status", "pending")).order("desc").take(100);
    }
...
export const approveDeposit = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const transactionId = await ctx.db.insert("transactions", {
      ...
      type: "credit",
      status: "completed",
      description: "Deposit approved by Crestline Capital operations",
...
export const approveWithdrawal = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const transactionId = await ctx.db.insert("transactions", {
      ...
      type: "debit",
      status: "completed",
      description: "Withdrawal approved by Crestline Capital operations",
      reference: request.reference,
      createdAt: now,
      updatedAt: now,
    });
